...
if [ "$1" = "help" ]; then
    ...
    if grep -E "^taskmanager\.host:.*" "${CONF_FILE}" > /dev/null; then
        sed -i -e "s/taskmanager\.host:.*/taskmanager\.host: ${HOSTNAME}/g" "${CONF_FILE}"
    else
        echo "taskmanager.host: ${HOSTNAME}" >> "${CONF_FILE}"
    fi
